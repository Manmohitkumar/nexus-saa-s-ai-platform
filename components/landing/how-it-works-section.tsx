...
const steps = [
  {
    number: "01",
    title: "Connect",
    description: "Sync GitHub, Slack, Notion, Jira, Confluence, Google Drive, APIs, and infrastructure sources.",
    code: `phoenix.ingest({
  source: "github",
  repository: "payments-service",
  extract: ["entities", "dependencies", "decisions"]
})`,
  },
  {
    number: "02",
    title: "Understand",
    description: "Extract entities, relationships, ownership, dependencies, and decision context into a living knowledge graph.",
    code: `phoenix.graph.build({
  extract: ["entities", "ownership"],
  link: ["dependencies", "decisions"]
})`,
  },
  {
    number: "03",
    title: "Act",
    description: "Search organizational memory, investigate risks, replay decisions, onboard engineers, and generate operational insights.",
    code: `phoenix.insight({
  query: "payment architecture risk",
  explain: true,
  include: ["decisions", "owners"]
})`,
  },
];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        {/* Header */}
        <div className="mb-20">
          <p className="text-sm font-mono text-primary mb-3">// HOW PHOENIX WORKS</p>
          <h2
          ...
          >
            ...
            <br />
            <span className="text-balance">organizational intelligence.</span>
          </h2>
        </div>
                    ...
                    >
                      ...
                      <span>
                        {highlightCode(line).map((token, tokenIndex) => (
                          <span key={`${activeStep}-${i}-${tokenIndex}`}>{token}</span>
                        ))}
                      </span>
                    </div>
                  ))}
...
function highlightCode(line: string): ReactNode[] {
  const parts = line.split(/(\/\/.*$|".*?"|'.*?'|\.\w+|\b(?:phoenix|graph|entities|decisions)\b|[{}()[\]:])/g);
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith(".")) return <span className="text-primary" key={index}>{part}</span>;
    if (/^(phoenix|graph|entities|decisions)$/.test(part)) {
      return <span className="text-foreground" key={index}>{part}</span>;
    }
    if (/^[{}()[\]:]$/.test(part)) return <span className="text-muted-foreground/70" key={index}>{part}</span>;
